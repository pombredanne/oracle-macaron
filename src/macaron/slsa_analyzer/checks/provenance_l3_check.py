...
class ProvenanceL3VerifiedFacts(CheckFacts):
    ...
    #: The verifier result to be shown as justification.
    #: Note that we do not include the name of artifacts (subjects) in the justification because 
    #: there can be too many of them.
    verifer_result: Mapped[str] = mapped_column(nullable=False, info={"justification": JustificationType.TEXT})
...
class ProvenanceL3Check(BaseCheck):
    ...
    def run_check(self, ctx: AnalyzeContext) -> CheckResultData:
        ...
        for ci_info in ci_services:
            ...
            try:
                ...
                return CheckResultData(
                    exit_justification=["Could not verify level 3 provenance."],
        ...
        if all_feedback:
            ...
            skipped = [
                feedback for feedback in all_feedback if feedback.verify_result.result not in [_VerifyArtifactResultType.FAILED, _VerifyArtifactResultType.PASSED]
            ]

            if failed or skipped:
                result_value = CheckResultType.FAILED
            else:
                result_tables.append(ProvenanceL3VerifiedFacts(verifer_result=_VerifyArtifactResultType.PASSED.value, confidence=Confidence.HIGH))
                result_value = CheckResultType.PASSED
                return CheckResultData(result_tables=result_tables, result_type=result_value)

        return CheckResultData(exit_justification=["Could not verify level 3 provenance."], result_tables=result_tables, result_type=result_value)
