...
class BuildAsCodeCheck(BaseCheck):
    ...
    def _check_build_tool(
        ...
        exit_justification: list[str]
            List of justifications to add to for failed checks.
        result_tables: ResultTables
                ...
                for unparsed_ci in (Jenkins, Travis, CircleCI, GitLabCI):
                    if isinstance(ci_service, unparsed_ci):
                        if build_tool.ci_deploy_kws[ci_service.name]:
                            ...
                            check_result = CheckResultType.PASSED
        if check_result == CheckResultType.FAILED:
            exit_justification.append(f"The target repository does not use {build_tool.name} to deploy.")
    ...
    def run_check(self, ctx: AnalyzeContext) -> CheckResultData:
        ...
        for tool in build_tools:
            res = self._check_build_tool(tool, ctx, ci_services, exit_justification, result_tables)
        ...
        return CheckResultData(
            exit_justification=exit_justification, result_tables=result_tables, result_type=overall_res
        )
